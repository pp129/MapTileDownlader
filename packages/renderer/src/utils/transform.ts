...
const sphericalMercator = {
  forward: forEachPoint(
    (input: Coordinate, output: Coordinate, offset: number) => {
      const lat = Math.max(
...
const gcj02 = {
  toWGS84: forEachPoint(
    (input: Array<number>, output: Array<number>, offset: number) => {
...
const bd09 = {
  toWGS84: (
    input: number[],
    opt_output: number[] | undefined,
    opt_dimension: number | undefined,
  ) => {
